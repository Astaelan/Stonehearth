...
{
    public sealed class GetPresenceFrame
    {
        public BattleNet.PresenceUpdate[] Updates = null;
        ...
        public GetPresenceFrame Read(BinaryReader pReader)
        {
            int updatesLength = pReader.ReadInt32();
            Updates = new BattleNet.PresenceUpdate[updatesLength];
        ...
        {
            pWriter.WriteLine("GetPresence");
            pWriter.Indent++;
            pWriter.WriteLine("Updates: {0}", Updates.Length);
            if (Updates.Length > 0)
            {
                pWriter.Indent++;
                foreach (BattleNet.PresenceUpdate value in Updates)
                {
